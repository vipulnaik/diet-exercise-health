...
queries = [
    ...
    "select * from food_preparations_and_openings_self_join where preparation_or_opening_date_2 is null and datediff(curdate(), completion_date_1) > 1 and not (food_type in ('Whole Foods Mexican Whole Wheat Tortillas','Udupi Palace spinach masala dosa','Lundberg Sustainable California White Jasmine Rice','Whole Foods Walnuts')) and not ((food_type, completion_date_1, completion_meal_index_1) = ('TJ Carb Savvy Tortillas','2025-03-23',2));",
    ...
    "select * from food_preparations_and_openings where food_type = 'Red bell pepper' and quantity != 2 and not (preparation_or_opening_date in ('2024-09-08','2024-11-20','2025-01-01','2025-01-07'));",
    ...
    "select epoch, daily_total_carb_in_grams, daily_calories from daily_pow_amounts_by_epoch where daily_total_carb_in_grams < 0.1 * daily_calories or daily_total_carb_in_grams > 0.16 * daily_calories;",
    ...
    "select epoch, daily_potassium_in_mg from daily_pow_amounts_by_epoch where daily_potassium_in_mg < 3200 or daily_potassium_in_mg > 5300;",
